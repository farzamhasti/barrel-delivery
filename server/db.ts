...
// System Credentials (Admin and Kitchen)
export async function getSystemCredentialByUsername(username: string) {
  const db = await getDb();
  if (!db) return null;
  
  const { systemCredentials } = await import("../drizzle/schema");
  
  const result = await db
    .select()
    .from(systemCredentials)
    .where(and(
      eq(systemCredentials.username, username),
...
export async function createSystemSession(credentialId: number, sessionToken: string, expiresAt: Date) {
  ...
  const { systemSessions } = await import("../drizzle/schema");
  
  return db.insert(systemSessions).values({
    credentialId,
    sessionToken,
    expiresAt,
  });
...
export async function getSystemSession(sessionToken: string) {
  const db = await getDb();
  if (!db) return null;
...
export async function getSystemCredentialBySessionToken(sessionToken: string) {
  const db = await getDb();
  if (!db) return null;
  
  const { systemSessions, systemCredentials } = await import("../drizzle/schema");
  
  const result = await db
    .select({
      credential: systemCredentials,
  ...
  return result[0]?.credential || null;
}
...
export async function deleteSystemSession(sessionToken: string) {
  ...
  return db.delete(systemSessions).where(eq(systemSessions.sessionToken, sessionToken));
}

export async function initializeSystemCredentials() {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot initialize system credentials: database not available");
    return;
  }
  ...
  const existing = await db.select().from(systemCredentials);
  if (existing.length > 0) {
    console.log("[Database] System credentials already initialized");
    return;
  }
  
  // Hash passwords
  const adminPasswordHash = await bcrypt.default.hash("Barrel_1981@", 10);
  const kitchenPasswordHash = await bcrypt.default.hash("1111", 10);
  ...
  console.log("[Database] System credentials initialized successfully");
}
