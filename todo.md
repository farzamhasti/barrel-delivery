...
## BUG FIX - Order Tracking Tab Shows No Orders on Mobile

- [x] Debug why Order Tracking page shows "Active Orders (0)" on mobile
- [x] Check if getActiveOrders query is being called on mobile
- [x] Verify orders are being fetched correctly from backend
- [x] Check if there's a responsive layout issue hiding orders
- [x] Test order tracking on desktop vs mobile
- [x] Ensure orders display correctly when available
- [x] Fixed layout by hiding map on mobile by default (window.innerWidth < 1024)
- [x] Added flex-shrink-0 to map container to prevent it from taking all space
